...
class Contributor(object):
	...
	def contact(self):
		"""Contact information"""
		return "%20s <%s>" % (self.name, ",".join(mail for mail, count in sorted(self.emails.iteritems(), key=lambda item: item[1], reverse=True)))
...
def get_contributors(aliases=None, source_file=None):
	...
	try:
		changes = subprocess.check_output(
			['git', 'log', '--format="%H|%aN|%aE"', '--no-merges', '-w', '--numstat'] + (['--follow', source_file] if source_file is not None else [])
		).splitlines()
	except subprocess.CalledProcessError as err:
		if err.returncode != 128:
			raise
...
if __name__ == "__main__":
	...
	if options.fetch_license and write_license(license_url=options.fetch_license, license_file=os.path.join(repo_base, "LICENSE")):
		print("Added LICENSE")
	else:
